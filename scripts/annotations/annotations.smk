...
rule Prodigal:
    ...
    output: 
        ...
        gff = homedir + "/output/{sample}/1.annotations/{sample}.gff"
    #conda: f"{Conda_env_dir}/phagesnake.yaml"
    log: log_dir + "/{sample}_annotations.log"
    shell: '''# 1 prodigal annotation
prodigal -i {input} -a {output.faa} -f gff -o {output.gff} -c -q -p meta > {log}
'''

rule EggNOG:
    input: homedir + "/output/{sample}/1.annotations/{sample}.faa"
    output: homedir + "/output/{sample}/1.annotations/eggnog/{sample}.emapper.annotations"
    threads: 50
    params:
        out_dir = homedir + '/output/{sample}/1.annotations/eggnog'
    log: log_dir + "/{sample}_annotations.log"
    #conda: f"{Conda_env_dir}/phagesnake.yaml"
    shell: '''# 1.1 EggNOG annotation
...
rule Diamond_blastp:
    input: homedir + '/output/{sample}/1.annotations/{sample}.faa'
    output: temp(homedir + '/output/{sample}/1.annotations/{sample}.dm.tsv')
    params: 
        vc2_db = f"{db_path}/{db_prefix}_vConTACT2_proteins.dmnd"
    threads: 60
    log: log_dir + "/{sample}_annotations.log"
    #conda: f"{Conda_env_dir}/phagesnake.yaml"
    shell: '''# 2.1 Diamond blastp All protein to find the annotation
...
rule filter_All_BLASTp:
    input: tsv = homedir + '/output/{sample}/1.annotations/{sample}.dm.tsv'
    output: fmt_blastp = homedir + '/output/{sample}/1.annotations/blastp_fmt.tsv'
    params: 
        ...
        cov = 70
    log: log_dir + "/{sample}_annotations.log"
    shell: '''# 2.2 parse the blastp All output
...
rule final_gbk:
    input:
        ...
        faa = homedir + "/output/{sample}/1.annotations/{sample}.faa",
        blastp = homedir + "/output/{sample}/1.annotations/blastp_fmt.tsv",
        eggnog = homedir + "/output/{sample}/1.annotations/eggnog/{sample}.emapper.annotations"
    output: homedir + "/output/{sample}/1.annotations/{sample}.gbk"
    log: log_dir + "/{sample}_annotations.log"
    #conda: f"{Conda_env_dir}/phagesnake.yaml"
    shell: '''# 3 fasta + Prodigal + EggNOG + blastp -> gbk
...
rule genome_visualize:
    input: homedir + "/output/{sample}/1.annotations/{sample}.gbk"
    output: 
        png_out = homedir + "/output/{sample}/1.annotations/{sample}.png",
        svg_out = homedir + "/output/{sample}/1.annotations/{sample}.svg"
    log: log_dir + "/{sample}_annotations.log"
    #conda: f"{Conda_env_dir}/phagesnake.yaml"
    shell: '''# 3.1 genome_viewer
...
rule abr:
    ...
    output: 
        ...
        abr_dir = directory(homedir + "/output/{sample}/1.annotations/ABRicate")
    log: log_dir + "/{sample}_abr_check.log"
    #conda: f"{Conda_env_dir}/phagesnake.yaml"
    threads: 8
    shell: '''# 4 abricate annotation
...
rule annotation_all:
    input:
        png_out = homedir + "/output/{sample}/1.annotations/{sample}.png",
        svg_out = homedir + "/output/{sample}/1.annotations/{sample}.svg",
        gbk = homedir + "/output/{sample}/1.annotations/{sample}.gbk",
        gff = homedir + "/output/{sample}/1.annotations/{sample}.gff",
        faa = homedir + "/output/{sample}/1.annotations/{sample}.faa",
        blastp = homedir + "/output/{sample}/1.annotations/blastp_fmt.tsv",
        eggnog_out = homedir + "/output/{sample}/1.annotations/eggnog/{sample}.emapper.annotations",

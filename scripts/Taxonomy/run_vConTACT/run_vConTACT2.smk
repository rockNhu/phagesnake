...
runvc_script_dir = script_dir + '/Taxonomy/run_vConTACT'
rule gene2genome:
    input: homedir + '/output/{sample}/1.annotations/{sample}.faa'
    output: homedir + '/output/{sample}/5.vConTACT_network/vcontact2/gene_to_genome.csv'
...
rule Diamond_blastp_raw:
    ...
    output: 
        sample_dmnd = homedir + '/output/{sample}/5.vConTACT_network/vcontact2/{sample}.dmnd',
        blp_f = homedir + '/output/{sample}/5.vConTACT_network/vcontact2/{sample}_f.dm.tsv',
        blp_r = homedir + '/output/{sample}/5.vConTACT_network/vcontact2/{sample}_r.dm.tsv',
        final_blp = homedir + '/output/{sample}/5.vConTACT_network/vcontact2/{sample}_bidir.dm.tsv'
    params: 
...
rule vConTACT2:
    input: 
        g2g = homedir + '/output/{sample}/5.vConTACT_network/vcontact2/gene_to_genome.csv',
        blp = homedir + '/output/{sample}/5.vConTACT_network/vcontact2/{sample}_bidir.dm.tsv'
    output:
        network = homedir + '/output/{sample}/5.vConTACT_network/c1.ntw',
        overview = homedir + '/output/{sample}/5.vConTACT_network/genome_by_genome_overview.csv'
    params:
        wk_dir = homedir + '/output/{sample}/5.vConTACT_network/vcontact2',
...
rule vConTACT_visualize:
    ...
    output: 
        small_network = homedir + '/output/{sample}/5.vConTACT_network/small_c1.ntw',
        small_overview = homedir + '/output/{sample}/5.vConTACT_network/small_genome_by_genome_overview.csv',
        small_database = homedir + '/output/{sample}/5.vConTACT_network/small_data.tsv',
        graph = homedir + "/output/{sample}/5.vConTACT_network/{sample}_vConTACT2.html"
    log: log_dir + "/{sample}_run_vConTACT2.log"
    threads: 60
    #conda: f"{Conda_env_dir}/phagesnake.yaml"
    params: outdir = homedir + '/output/{sample}/5.vConTACT_network/graphanalyze_'
...
rule vcontact_all:
    ...
    output:
        ...
        db_data = homedir + '/output/{sample}/5.vConTACT_network/' + f'{db_prefix}_data.tsv'
    params:
        workdir = homedir + "/output/{sample}/5.vConTACT_network/vcontact2"
